...
//    private MyStatistics st = MyStatistics.getMyStatisticsObject();
     ...
     * @param firstSlot first slot of the created lightpath
     * @param lastSlot last slot of the created lightpath
     * @return -1 if LightPath object cannot be created, or its unique identifier otherwise
     */
    public long createLightpath(int[] links, int firstSlot, int lastSlot) {
        LightPath lp;
        int src, dst;
        long id;
        if (links.length < 1) {
            ...
            if (!canCreateLightpath(links, firstSlot, lastSlot)) {
                return -1;
            }
            createLightpathInPT(links, firstSlot, lastSlot);
            src = pt.getLink(links[0]).getSource();
            dst = pt.getLink(links[links.length - 1]).getDestination();
            id = this.nextLightpathID;
            lp = new LightPath(id, src, dst, links, firstSlot, lastSlot);
    ...
    public boolean removeLightPath(long id) {
        ...
        if (id < 0) {
            ...
            removeLightpathFromPT(lp.getLinks(), lp.getFirstSlot(), lp.getLastSlot());
            src = lp.getSource();
     ...
     * @param links list of integers that represent the links that form the lightpath
     * @param firstSlot first slot of the rerouted lightpath
     * @param lastSlot last slot of the rerouted lightpath
     * @return true if operation was successful, or false otherwise
     */
    public boolean rerouteLightPath(long id, int[] links, int firstSlot, int lastSlot) {
        int src, dst;
        LightPath old, lp;
        if (links.length < 1 || firstSlot > lastSlot) {
            throw (new IllegalArgumentException());
        } else {
            if (!lightPaths.containsKey(id)) {
                return false;
            ...
            removeLightpathFromPT(old.getLinks(), old.getFirstSlot(), old.getLastSlot());
            if (!canCreateLightpath(links, firstSlot, lastSlot)) {
                createLightpathInPT(old.getLinks(), old.getFirstSlot(), old.getLastSlot());
                return false;
            }
            createLightpathInPT(links, firstSlot, lastSlot);
            src = pt.getLink(links[0]).getSource();
     ...
     *
     * @param links list of integers that represent the links that form the lightpath
     * @param firstSlot first slot of the verified lightpath
     * @param lastSlot last slot of the verified lightpath
     * @return true if the lightpath can be created, or false otherwise
     */
    public boolean canCreateLightpath(int[] links, int firstSlot, int lastSlot) {
        try {
	        for (int i = 0; i < links.length; i++) {
	            if (!pt.getLink(links[i]).areSlotsAvailable(firstSlot, lastSlot)) {
	                return false;
	            }
	        }
     ...
     */
    private void createLightpathInPT(int[] links, int firstSlot, int lastSlot) {
        for (int i = 0; i < links.length; i++) {
            pt.getLink(links[i]).reserveSlots(firstSlot, lastSlot);
        }
    }
     ...
     * @param firstSlot list of wavelength values used in the lightpath links
     */
    private void removeLightpathFromPT(int[] links, int firstSlot, int lastSlot) {
        for (int i = 0; i < links.length; i++) {
            pt.getLink(links[i]).releaseSlots(firstSlot, lastSlot);
        }
    }
